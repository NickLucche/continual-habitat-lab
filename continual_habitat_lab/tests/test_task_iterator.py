...
from continual_habitat_lab.config import ContinualHabitatLabConfig, TASK_CHANGE_BEHAVIOR
...
def test_no_tasks():
    ...
    assert len(ti.tasks) == 1 and isinstance(ti.tasks[0], VoidTask) 
...
def test_multiple_tasks_from_config():
    ...
    cfg = ContinualHabitatLabConfig(OmegaConf.create(cfg), from_cli=False)
    ti = TaskIterator(cfg, None)
